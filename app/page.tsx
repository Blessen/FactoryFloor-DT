  ...
  return (
    <main className="flex h-screen w-screen flex-col bg-[#090d16] text-white overflow-hidden font-sans relative">
      ...
      <div className="flex-grow flex w-full overflow-hidden z-10 relative">
        ...
        <div className={`relative transition-all duration-300 ease-in-out flex-shrink-0 z-20 ${isSidebarCollapsed ? 'w-0' : 'w-[350px]'
          }`}>
          <aside className={`h-full w-full border-r border-slate-800/80 bg-slate-950/60 backdrop-blur-md flex flex-col gap-5 overflow-y-auto transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'p-0 border-r-0 opacity-0 pointer-events-none' : 'p-5 opacity-100'
            }`}>
